...
## Microdades original ---------------------------
dades <- CEOdata::CEOdata(reo = "1110", raw = TRUE) |>
...
dades_raking <- dades %>% 
  dplyr::select(IDENTIFICADOR_COMPLET, MODE_ADMIN, ELECTOR, SEXE, EDAT_GR, LLOC_NAIX, LLENGUA_PRIMERA_1_3, ESTUDIS_1_6, CLUSTER21, HABITAT, PROVINCIA)
```
...
poblacio_dist <- poblacio_dist %>% 
  ...
  rename(valor = esp_18, # Opcions: res_16, res_18, esp_18, esp_18
...
dades_raking$PONDERA_ELECTORS <- weights(dades_svy_rake_trim10)
```

```{r save .sav results}
## Guardem la matriu .sav ----------------------------------------
write_sav(dades_raking, file.path(DTA_FOLDER, "PONDERA_ELECTORS_SP.sav"))
```
