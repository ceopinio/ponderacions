...
```{r carrega llibreries, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
library(pacman)
library(tidyverse)
...
## Microdades original ---------------------------
dades <- CEOdata::CEOdata(reo = "1093", raw = TRUE)
...
dades_imputed <- dades_raking %>% 
    ...
    impute_vars(to_impute = c("SEXE_EDAT_SVY", "LLOC_NAIX_SVY", "LLENGUA_PRIMERA_SVY", "ESTUDIS_SVY", "CLUSTER21_SVY", "HABITAT", "PROVINCIA"), seed = 1903)
...
write_sav(dades_raking, file.path(RUTA_PC, "ponderacions/PONDERA_ENLINIA_Panel_Ciutada.sav"))
```