...
dades_onada_2 <- CEOdata::CEOdata(reo = "1118", raw = TRUE) |> 
  filter(CIUTADANIA %in% c(1,2) & EDAT >= 18) |> 
  mutate(ELECTOR = 1)
...
Ens quedem amb les dades poblacionals de referència pel nostre marc mostral (residents de 16 anys i més) més recents. En el cas de PONDERA_ELECTOR agafem només les persones amb ciutadania espanyola i majors de 18 anys.
...
poblacio_dist <- poblacio_dist |> 
  ...
  rename(valor = esp_18, # Opcions: res_16, res_18, esp_15, esp_18
         resposta = resposta_18) |> # Opcions: resposta_16, resposta_18
...
dades_raking$PONDERA_ELECTOR <- weights(dades_svy_rake_trim10)

## Fem que la mitjana sumi exactament 1
dades_raking$PONDERA_ELECTOR <- dades_raking$PONDERA_ELECTOR / mean(dades_raking$PONDERA_ELECTOR)
```

```{r save .sav results}
## Guardem la matriu .sav ----------------------------------------
write_sav(dades_raking, file.path(DTA_FOLDER, "PONDERA_ELECTOR_longi_v2.sav"))
```
